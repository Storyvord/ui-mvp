...
import { Contracts, Script, Sheet } from "./icons/docsIcons";
import { Button } from "@/components/ui/button";
import RoomPage from "@/components/file-documents/RoomPage";
import ContractsPage from "@/components/file-documents/ContractsPage";
import ScriptsPage from "@/components/file-documents/ScriptsPage";
import Tabs from "@/components/file-documents/Tabs";
import CallSheetsPage from "@/components/file-documents/CallSheetsPage";
...
const File: FC = () => {
  ...
  const [changingIconForRoomIndex, setChangingIconForRoomIndex] = useState<
    number | null
  >(null);
  ...
  const [files] = useState<RoomDataType[]>([
    ...
    },
    {
      ...
      description:
        "You can find copies of your call sheets here after sending them to your crew.",
    },
  ]);
  ...
  const handleDeleteRoom = (index: number) => {
    if (index < files.length) {
      ...
      setCreatedRooms((rooms) =>
        rooms.filter((_, i) => i !== index - files.length)
      );
    }
  };
  ...
  const handleChangeIcon = (index: number, newIcon: React.ElementType) => {
    if (index < files.length) {
      ...
      setCreatedRooms((rooms) =>
        rooms.map((room, i) =>
          i === index - files.length ? { ...room, icon: newIcon } : room
        )
      );
    }
  };
  ...
  const RoomComponent =
    currentRoomId && currentRoomId.startsWith("user-room")
      ? RoomPage
      : currentRoomId
      ? pages[currentRoomId]
      : RoomPage;
  ...
  const allTabs = ["Home"].concat(
    isHome ? [] : defaultTabs.concat(createdRooms.map((room) => room.title))
  );
      ...
      <Tabs
        ...
        setActiveTab={(tab) => {
          ...
          if (tab === "Home") {
            ...
            const room = [...files, ...createdRooms].find(
              (room) => room.title === tab
            );
            if (room) {
              setIsHome(false);
                      ...
                      {selectedRoomIndex === index && (
                        ...
                        >
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleOpenChangeIconModal(index);
                            }}
                          >
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center shadow-lg">
          <div className="bg-white p-8 rounded-lg w-96 max-h-screen transform transition-transform duration-300 shadow-xl">
                ...
                <div
                  ...
                  className={`p-2 rounded cursor-pointer ${
                    selectedIconIndex === index
                      ? "text-black"
                      : "text-slate-400"
                  } ${selectedIconIndex === index && "hover:opacity-75"}`}
                >
                  <Icon />
                </div>
              ))}
            ...
            <footer className="pt-4 flex justify-end space-x-3 mt-1">
              <Button
                onClick={handleCloseChangeIconModal}
                type="button"
                variant="outline"
              >
                Close
