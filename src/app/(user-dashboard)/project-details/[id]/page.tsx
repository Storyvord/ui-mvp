...
import { useCompleteProject, useDeleteProject, useProjectDetails } from "@/lib/react-query/queriesAndMutations";
...
const ProjectPage = ({params}: {params: {id: string;}}) => {
  ...
  const {data: projectDetails, isLoading: projectDetailsLoading, error} = useProjectDetails(params.id);
  const {mutateAsync: deleteProject, isLoading:deletingProject} = useDeleteProject();
  const {mutateAsync: completeProject, isLoading:completingProject} = useCompleteProject(params.id);
  ...
  if(error){
    return(
      <div className="w-full text-center text-gray-700">Failed to fetch project details</div>
    )
  }
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    remove your project data from our servers.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                  <Button onClick={handleDeleteProject} className={`${deletingProject? "disabled" : ""}`}>
